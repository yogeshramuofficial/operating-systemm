...
int main() {
    int req[8]={98,183,41,122,14,124,65,67}, head=53, total=0, left[9], right[9], l=0, r=0, disk=200;

    for(int i=0;i<8;i++){
        if(req[i]<head) left[l++]=req[i]; else right[r++]=req[i];
    }
    ...
    for(int i=l-1;i>=0;i--){int s=abs(head-left[i]); total+=s; printf("%d->%d, Seek=%d\n", head,left[i],s); head=left[i];}
